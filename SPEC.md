# SPEC: MyBriefcase Bookmarks — Local-First Bookmark Manager with Automerge over Syncthing
...
| Mode | `MBB_DEV_MODE` | `MBB_CLIENT_ID` | Result |
|---|---|---|---|
| Normal | unset | unset | bare hostname |
| Normal | unset | set | `MBB_CLIENT_ID` value |
| Dev | set | unset | hostname + `-` + random 8-char suffix (ephemeral) |
| Dev | set | set | `MBB_CLIENT_ID` value |
...
pub fn init_repo(
    ...
    let (doc_handle, document_id) = if sync_info_path.exists() {
        ...
        let info = serde_json::json!({
            ...
            "app": "mybriefcase-bookmarks", "schema_version": 1,
