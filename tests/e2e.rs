...
impl ServerProcess {
    async fn start(
        ...
        let child = Command::new(binary)
            .env("BOOKMARK_SYNC_ROOT", sync_root)
            .env("BOOKMARK_LOCAL_DIR", local_dir)
            .env("BOOKMARK_PORT", port.to_string())
            .env("BOOKMARK_CLIENT_ID", client_id)
...
async fn live_bidirectional_sync() {
    let binary = env!("CARGO_BIN_EXE_automerge-playground");
