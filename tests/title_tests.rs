...
use automerge_playground::views;
use axum::Router;
use axum::body::Body;
...
fn build_html_app(
    ...
    let state = Arc::new(automerge_playground::api::AppState {
        doc_handle,
        sync_root,
        client_id,
        sse_tx,
...
async fn full_page_root_has_title() {
    ...
    assert!(
        html.contains("<title>Bookmark Manager</title>"),
        "Root page should have 'Bookmark Manager' title"
    );
}
...
async fn full_page_folder_has_dynamic_title() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(
        html.contains("<title>Work Stuff — Bookmark Manager</title>"),
...
async fn htmx_folder_response_has_title_tag() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(
        html.contains("<title>My Folder — Bookmark Manager</title>"),
...
async fn htmx_root_folder_response_has_title_tag() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(
        html.contains("<title>Bookmarks — Bookmark Manager</title>"),
...
async fn title_escapes_html_entities() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(
        html.contains("<title>A&lt;B&amp;C — Bookmark Manager</title>"),
