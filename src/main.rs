use automerge_playground::{api, identity, repo, views, watcher};
    ...
    fn from_env() -> Self {
        Self {
            sync_root: PathBuf::from(
                std::env::var("BOOKMARK_SYNC_ROOT").unwrap_or_else(|_| "./sync_data".to_owned()),
            ),
            local_data_dir: PathBuf::from(
                std::env::var("BOOKMARK_LOCAL_DIR").unwrap_or_else(|_| "./local_data".to_owned()),
            ),
            port: std::env::var("BOOKMARK_PORT")
                .ok()
                .and_then(|p| p.parse().ok())
                .unwrap_or(3000),
            dev_mode: std::env::var("BOOKMARK_DEV_MODE").is_ok(),
            client_id: std::env::var("BOOKMARK_CLIENT_ID").ok(),
        }
    }
}
