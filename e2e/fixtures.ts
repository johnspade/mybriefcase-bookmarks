...
function getBinaryPath(): string {
  ...
  try {
    ...
    return path.join(meta.target_directory, 'release', 'mybriefcase-bookmarks');
  } catch {
    return path.join(PROJECT_ROOT, 'target', 'release', 'mybriefcase-bookmarks');
  }
}
...
export const test = base.extend<ServerFixture>({
  serverPage: [async ({ browser }, use) => {
    ...
    const serverProcess: ChildProcess = spawn(binaryPath, [], {
      env: {
        ...process.env,
        MBB_PORT: String(port),
        MBB_SYNC_ROOT: syncRoot,
        MBB_LOCAL_DIR: localDir,
        MBB_DEV_MODE: '1',
        MBB_CLIENT_ID: 'e2e-test',
      },
      cwd: PROJECT_ROOT,
